...
namespace streamer::objects
{
    ...
    Item::SharedObject CreateDynamicObject(std::optional<AMX*> amx, int modelId, const Eigen::Vector3f& position, const Eigen::Vector3f& rotation, int worldId = -1, int interiorId = -1, int playerId = -1, float streamDistance = STREAMER_OBJECT_SD, float drawDistance = STREAMER_OBJECT_DD, int areaId = -1, int priority = 0);
    Item::SharedObject CreateDynamicObjectEx(std::optional<AMX*> amx, int modelId, const Eigen::Vector3f& position, const Eigen::Vector3f& rotation, float streamDistance = STREAMER_OBJECT_SD, float drawDistance = STREAMER_OBJECT_DD, const std::unordered_set<int>& worlds = {}, const std::unordered_set<int>& interiors = {}, const std::unordered_set<int>& players = {}, const std::unordered_set<int>& areas = {}, int priority = 0);
