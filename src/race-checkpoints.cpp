...
namespace streamer::racecheckpoints
{
    ...
    Item::SharedRaceCheckpoint CreateDynamicRaceCheckpoint(std::optional<AMX*> amx, int type, const Eigen::Vector3f& position, const Eigen::Vector3f& next, float size, int worldId, int interiorId, int playerId, float streamDistance, int areaId, int priority)
    {
    ...
    Item::SharedRaceCheckpoint CreateDynamicRaceCheckpointEx(std::optional<AMX*> amx, int type, const Eigen::Vector3f& position, const Eigen::Vector3f& next, float size, float streamDistance, const std::unordered_set<int>& worlds, const std::unordered_set<int>& interiors, const std::unordered_set<int>& players, const std::unordered_set<int>& areas, int priority)
    {
        if (core->getData()->getGlobalMaxItems(STREAMER_TYPE_RACE_CP) == core->getData()->raceCheckpoints.size())
        {
            return nullptr;
