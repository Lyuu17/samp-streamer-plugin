...
namespace streamer::actors
{
    ...
    Item::SharedActor CreateDynamicActor(std::optional<AMX*> amx, int modelId, const Eigen::Vector3f& position, float rotation, bool invulnerable = true, float health = 100.0, int worldId = -1, int interiorId = -1, int playerId = -1, float streamDistance = STREAMER_ACTOR_SD, int areaId = -1, int priority = 0);
    Item::SharedActor CreateDynamicActorEx(std::optional<AMX*> amx, int modelId, const Eigen::Vector3f& position, float rotation, bool invulnerable = true, float health = 100.0, float streamDistance = STREAMER_ACTOR_SD, const std::unordered_set<int>& worlds = {}, const std::unordered_set<int>& interiors = {}, const std::unordered_set<int>& players = {}, const std::unordered_set<int>& areas = {}, int priority = 0);
