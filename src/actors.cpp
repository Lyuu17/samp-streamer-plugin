...
namespace streamer::actors
{
    ...
    Item::SharedActor CreateDynamicActor(std::optional<AMX*> amx, int modelId, const Eigen::Vector3f& position, float rotation, bool invulnerable, float health, int worldId, int interiorId, int playerId, float streamDistance, int areaId, int priority)
    {
    ...
    Item::SharedActor CreateDynamicActorEx(std::optional<AMX*> amx, int modelId, const Eigen::Vector3f& position, float rotation, bool invulnerable, float health, float streamDistance, const std::unordered_set<int>& worlds, const std::unordered_set<int>& interiors, const std::unordered_set<int>& players, const std::unordered_set<int>& areas, int priority)
    {
        if (core->getData()->getGlobalMaxItems(STREAMER_TYPE_ACTOR) == core->getData()->actors.size())
        {
            return nullptr;
