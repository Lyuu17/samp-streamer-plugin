...
namespace streamer::textlabels
{
    ...
    Item::SharedTextLabel CreateDynamicTextLabel(std::optional<AMX*> amx, const std::string& text, int color, const Eigen::Vector3f& position, float drawDistance, int attachedPlayerId = -1, int attachedVehicleId = -1, bool testLOS = false, int worldId = -1, int interiorId = -1, int playerId = -1, float streamDistance = STREAMER_3D_TEXT_LABEL_SD, int areaId = -1, int priority = 0);
    Item::SharedTextLabel CreateDynamicTextLabelEx(std::optional<AMX*> amx, const std::string& text, int color, const Eigen::Vector3f& position, float drawDistance, int attachedPlayerId, int attachedVehicleId, bool testLOS, float streamDistance, const std::unordered_set<int>& worlds, const std::unordered_set<int>& interiors, const std::unordered_set<int>& players, const std::unordered_set<int>& areas, int priority);
