...
namespace streamer::objects
{
    ...
    Item::SharedObject CreateDynamicObject(std::optional<AMX*> amx, int modelId, const Eigen::Vector3f& position, const Eigen::Vector3f& rotation, int worldId, int interiorId, int playerId, float streamDistance, float drawDistance, int areaId, int priority)
    {
    ...
    Item::SharedObject CreateDynamicObjectEx(std::optional<AMX*> amx, int modelId, const Eigen::Vector3f& position, const Eigen::Vector3f& rotation, float streamDistance, float drawDistance, const std::unordered_set<int>& worlds, const std::unordered_set<int>& interiors, const std::unordered_set<int>& players, const std::unordered_set<int>& areas, int priority)
    {
        if (core->getData()->getGlobalMaxItems(STREAMER_TYPE_OBJECT) == core->getData()->objects.size())
        {
            return nullptr;
